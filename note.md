...
第一题就是找一个文件。说实话开始连 `cd` 干啥都不知道，跟着做一遍才知道是切换目录。`chmod +x` 是给脚本加可执行权限；`grep -R` 是递归搜索；`ls -la` 能看到隐藏文件；`mkdir -p` 是建目录；`echo` 是把内容写进文件。这一堆命令一开始真的看不懂，做完一遍才大致明白。
...
`grep -R -l` 这次又用了一遍，比 Task 1 更熟了，知道 `-l` 是只显示文件名不显示内容。`sort -u` 是排序再去重。管道 `|` 这下彻底搞明白了，就是把上一个的输出接给下一个当输入。`cat` 也知道干啥了，就是把文件内容打到屏幕上。
...
这题是 1-4 里面最折腾的一题。要数 ERROR 总数、列出所有出现过的用户、再找出最常出现的错误码。一开始真不知道从哪下手，后来才知道用 `wc -l` 数行数；`cut` 是切割字符串（`-d ' '` 按空格切，`-d '='` 按等号切）；统计次数这块要 `sort | uniq -c | sort -nr | head -1` 一长串——先排序，`uniq -c` 把相邻行合并并加次数，再按数字倒序排，取第一个。这套打下来感觉特别爽，一口气写完感觉很对劲。
...
这题主要搞清楚 stdout 和 stderr 是两条不同的路。`>` 默认接的是 stdout；接 stderr 要用 `2>`。`tee` 比较特别，输出同时到屏幕和文件。除了这三个还顺带看了下 `tee -a`（追加，不覆盖）和 `2>&1`（把 stderr 合并到 stdout），这两招虽然这题没用到，但感觉以后应该会用到。
...
bug 的原因主要就是没加引号。在 shell 里，变量展开后默认会被再拆一次，如果文件名里有空格就会被当成两个文件。带上双引号就会当成一个整体。这次改完我记住一条：能加引号的地方都加。
...
这题学的是怎么管进程。`pgrep`、`ps` 都能查进程；pid 就是进程的编号。`kill` 有几种信号：`-15` 是正常退出，`-9` 是直接强杀，`-0` 不发任何信号，只是用来探测进程还活着没。还学了 `&` 是把命令丢到后台跑，`jobs` 是看当前终端已经启动的后台任务。

---

## 总体感悟

几天的题做下来，shell 没有想象中那么玄，但也没那么简单。一开始看到那些命令是真的头大，连 `ls` 是干啥的都不清楚，跟着做几道之后慢慢能猜出一些命令大概是干啥的了，再做几道又熟悉了一轮。`grep`、`cut`、`sort`、`uniq` 这几个现在已经能闭眼敲出来；`PATH`、`PID`、stdout 和 stderr 是什么意思也大致知道。后面想试一下不靠笔记完整做一道题，看看自己到底掌握了多少。
