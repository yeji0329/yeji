...
对 bug 进行了修改。重点了解了：在 Shell 中，变量展开后默认会被**重新分词**，而引号可以阻止这次分词、把变量视作一个整体。
...
经过几天的做题并搭配 AI 讲解「食用」，我初步学会了如何在终端执行一些常规任务。从一开看见一串密密麻麻的英语就犯困，到现在能大致理解每行的意思，这个过程让我把基础一点一点打了起来。虽然现在还不能完整理解一些写法，但大致的意思已经懂了。也希望在不久的将来，能完完整整地自己独立在终端完成一次任务。
...
| 提交工具 | GitHub Git Data API（绕过本地 git push 凭据/网络限制） |
